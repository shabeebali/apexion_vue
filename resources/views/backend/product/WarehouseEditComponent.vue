<template>
    <div>
        <v-dialog v-model="dialog" lazy persistent>
            ...
            <v-card v-else>
                <v-card-title>
                    <span class="headline">Edit Warehouse: {{formdata.name.value}}</span>
                </v-card-title>
                <v-card-text>
                ...
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn color="blue darken-1" text @click="$emit('close-warehouse-edit-dialog')">Close</v-btn>
...
export default{
    ...
    methods:{
        saveItem(){
            ...
            axios.post('products/warehouse/edit/'+this.id,fD).then((response)=>{
                if(response.status == 200){
                    if(response.data.message == 'success'){
                        this.success=true
                        setTimeout(()=>{
                            this.$emit('close-warehouse-edit-dialog')
                            this.success=false
                        },600);
                        this.resetForm()
                    }
