<template>
    <div>
        <v-dialog v-model="dialog" lazy persistent>
            ...
            <v-card v-else>
                <v-card-title>
                    <span class="headline">Add Pricelist</span>
                </v-card-title>
                <v-card-text>
                ...
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn color="blue darken-1" text @click="$emit('close-pricelist-add-dialog');resetForm();">Close</v-btn>
...
export default{
    ...
    methods:{
        saveItem(){
            ...
            axios.post('products/pricelist/add',fD).then((response)=>{
                if(response.status == 200){
                    if(response.data.message == 'success'){
                        this.success=true
                        setTimeout(()=>{
                            this.$emit('close-pricelist-add-dialog')
                            this.success=false
                        },600);
