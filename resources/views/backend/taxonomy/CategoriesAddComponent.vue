    ...
    <div>
        <v-dialog v-model="dialog" lazy persistent>
            ...
            <v-card v-else>
                ...
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn color="blue darken-1" text @click="$emit('close-add-dialog');resetForm();">Close</v-btn>
                    <v-btn color="blue darken-1" text :loading="btnLoading" @click="saveItem">Save</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
    </div>
...
export default{
    ...
    methods:{
        saveItem(){
            ...
            axios.post('products/categories/add',fD).then((response)=>{
                if(response.status == 200){
        ...
        resetForm(){
            this.formdata = {
                ...
                'type_id':{
                    'value':this.category_type.options[0]['value']
                },
            }
        }
    }
