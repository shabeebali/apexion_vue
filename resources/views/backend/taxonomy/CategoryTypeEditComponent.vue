<template>
    <div>
        <v-dialog v-model="dialog" lazy persistent>
            ...
            <v-card v-else>
                <v-card-title>
                    <span class="headline">Edit Category Type: {{formdata.name.value}}</span>
                </v-card-title>
                <v-card-text>
                    <v-container grid-list-md>
                        <v-layout wrap>
                            ...
                            <v-flex xs12>
                                <v-select label="Code Length" disabled :value="formdata.code_length.value" :items="code_length.options"></v-select>
                            </v-flex>
                            <v-flex xs12>
                                <v-switch disabled :value ="formdata.autogen.value" true-value="1" false-value="0" label="Auto Generate?"></v-switch>
                            </v-flex>
                            <v-flex xs12>
                                <v-select disabled :value="formdata.code_type.value" label="Code Type" :items="code_type[formdata.code_length.value]['options']"></v-select>
                            </v-flex>
                            <v-flex xs12>
                                <v-switch disabled :value="formdata.in_pc.value" true-value="1" false-value="0" label="Include in Product Code?"></v-switch>
                            </v-flex>
                        </v-layout>
                    </v-container>
...
export default{
    ...
    methods:{
        saveItem(){
            ...
            axios.post('products/category_type/edit/'+this.id,fD).then((response)=>{
                if(response.status == 200){
                    if(response.data.message == 'success'){
                        this.success=true
                        setTimeout(()=>{
                            this.$emit('close-category-type-edit-dialog')
                            this.success=false
                        },600);
                        this.resetForm()
                    }
