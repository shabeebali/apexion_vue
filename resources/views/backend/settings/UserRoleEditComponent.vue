<template>
	<div>
		<v-content>
            <v-container fluid>
            	<v-card class="pa-2">
            		<v-subheader class="headline">Edit User Role</v-subheader>
            		<v-layout row wrap>
            			<v-flex xs12 px-4>
...
<script >
	export default{
		...
		mounted(){
			axios.get('/settings/users/roles/edit/'+this.$route.params.id).then((response)=>{
				Object.keys(response.data.permissions).forEach((key)=>{
					if(response.data.permissions[key].value == '1'){
						response.data.permissions[key].value = true
					}
					else{
						response.data.permissions[key].value = false
					}
				})
		...
		methods:{
			save(){
				...
				axios.post('/settings/users/roles/edit/'+this.$route.params.id,fD).then((response)=>{
