<template>
    <div>
        <v-layout  row mb-2>
            <v-subheader class="headline">Permissions</v-subheader>
            <v-spacer></v-spacer>
            <v-btn class="primary" @click.stop="addModel=true">Add</v-btn>
        </v-layout>
        <data-table :tableItems="tableItems" :tableHeaders="tableHeaders"></data-table>
        <add-component
...
export default{
    ...
    created(){
        this.getList();
        axios.get('/acl/permission/addconfig').then((response)=>{
    ...
    methods:{
        getList(){
            axios.get('/acl/permissions').then((response)=>{
                if(response.status == 200){
                    this.tableItems = response.data.permissions;
                    this.tableHeaders = response.data.headers;
                }
                else{
                    console.log('Something went wrong at settings/acl/PermissionsComponent '+response.status);
                }
            }).catch((error)=>{
                console.log('Something went wrong at settings/acl/PermissionsComponent '+error);
            });
        }
    }
}
